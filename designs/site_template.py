      ...
      <div class="panel">
        ...
        <div class="row" id="sideRow">
          <div class="seg" id="sideSeg">
            <button data-side="front">Front</button>
            <button data-side="back" class="on">Back</button>
          </div>
          <span class="hint" id="sideHint" style="margin:0;"></span>
        </div>
        <div class="row" id="frontStyleRow" style="display:none;">
          <div class="seg" id="frontSeg">
            <button data-fs="centered" class="on">Centered</button>
            <button data-fs="left_chest">Left chest</button>
            <button data-fs="concept">Hs &rarr; H+</button>
          </div>
        </div>
        ...
        <div class="row" style="margin-top:14px;">
          ...
          <button class="btn btn-primary" id="downloadStickerBtn" style="display:none;">Download Sticker</button>
...
let garment = "tee";
let designSide = "back";   // "front" (HUMAN+ logo) or "back" (stack art)
let frontStyle = "centered";   // centered | left_chest | concept
...
/* simplified STICKER variant (mirrors sticker_tile in generate_tiles.py) */
function stickerSVG(e){
  const acc=accentOf(e), W=1200, bx=90, by=90, bw=W-180, bh=W-180, cx=W/2;
  let tlBig,unit,famWord,trExtra="";
  if(e.fam==="PEP"){tlBig=e.name_num?e.name_num:(""+e.count);unit="AMINO ACIDS";famWord="PEPTIDE";}
  else{tlBig=""+e.count;unit="g/mol";famWord=e.fam==="MOL"?"MOLECULE":"HORMONE";
    trExtra=`<text x="${bx+bw-40}" y="${by+150}" font-family="${SANS}" font-size="40" font-weight="700" text-anchor="end" fill="${acc}">${esc(e.formula)}</text>`;}
  const nameFs=fitFS(e.full,bw-90,88);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${W}">
    ...
    <line x1="${bx}" y1="${by+210}" x2="${bx+bw}" y2="${by+210}" stroke="${acc}" stroke-width="8" stroke-opacity=".85"/>
    <text x="${bx+45}" y="${by+125}" font-family="${MONO}" font-size="96" font-weight="700" fill="${acc}">${tlBig}</text>
    <text x="${bx+50}" y="${by+180}" font-family="${MONO}" font-size="34" font-weight="700" letter-spacing="3" fill="#fff" fill-opacity=".85">${unit}</text>
    <text x="${bx+bw-40}" y="${by+70}" font-family="${MONO}" font-size="36" font-weight="700" letter-spacing="3" text-anchor="end" fill="#fff" fill-opacity=".85">${famWord}</text>
    ${trExtra}
    <text x="${cx}" y="${by+540}" font-family="${SANS}" font-size="360" font-weight="800" text-anchor="middle" fill="#fff">${esc(e.sym)}</text>
    <text x="${cx}" y="${by+678}" font-family="${SANS}" font-size="${nameFs}" font-weight="800" text-anchor="middle" fill="${acc}">${esc(e.full)}</text>
    <text x="${cx}" y="${by+730}" font-family="${SANS}" font-size="34" font-weight="700" letter-spacing="2" text-anchor="middle" fill="#fff" fill-opacity=".85">${esc(e.sub.toUpperCase())}</text>
    <text x="${cx}" y="${by+790}" font-family="${MONO}" font-size="38" font-weight="700" letter-spacing="4" text-anchor="middle" fill="#fff">${esc(e.tag)}</text>
  </svg>`;
}
/* the single selected element, if exactly one plain element is chosen (for stickers) */
function soloElement(){
  if(selected.length===1){const e=EL[selected[0]]; if(!e.blend) return e;}
  return null;
}
...
function stackTileSVG(name,sub,comps,tag,extras){
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1400">
    ...
    <text x="600" y="210" font-family="${MONO}" font-size="26" text-anchor="middle" letter-spacing="8" fill="#fff" fill-opacity=".45">HUMAN+ STACK SERIES</text>
    <text x="600" y="370" font-family="${SANS}" font-size="${nameFS}" font-weight="800" text-anchor="middle" fill="url(#sg)">${esc(name||"MY STACK")}</text>
    <text x="600" y="450" font-family="${MONO}" font-size="34" letter-spacing="6" text-anchor="middle" fill="#fff" fill-opacity=".75">${esc((sub||"CUSTOM PROTOCOL").toUpperCase())}</text>
    ${minis}
    ${extrasSVG}
    <text x="600" y="1090" font-family="${MONO}" font-size="40" font-weight="700" letter-spacing="8" text-anchor="middle" fill="#fff">${esc(tag||"BUILT DIFFERENT")}</text>
...
/* shared FRONT print: HUMAN+ logo, three placements (mirrors front_design) */
function frontDesignSVG(style){
  style = style || frontStyle;
  if(style==="left_chest"){
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1400">
      <text x="250" y="430" font-family="${SANS}" font-size="74" font-weight="800" letter-spacing="3" fill="#fff">HUMAN<tspan fill="${DB.green}">+</tspan></text>
      <text x="252" y="470" font-family="${MONO}" font-size="19" letter-spacing="6" fill="#fff" fill-opacity=".5">MODIFIED &#183; ENHANCED &#183; OPTIMIZED</text>
    </svg>`;
  }
  if(style==="concept"){
    const grey="#6b6b78";
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1400">
      <text x="600" y="300" font-family="${SANS}" font-size="100" font-weight="800" text-anchor="middle" letter-spacing="6" fill="#fff">HUMAN<tspan fill="${DB.green}">+</tspan></text>
      <text x="600" y="370" font-family="${MONO}" font-size="30" text-anchor="middle" letter-spacing="6" fill="#fff" fill-opacity=".5">FACTORY DEFAULT IS NOT FINAL</text>
      <rect x="150" y="500" width="380" height="420" rx="18" fill="none" stroke="${grey}" stroke-width="9"/>
      <text x="180" y="575" font-family="${MONO}" font-size="40" fill="${grey}">00</text>
      <text x="340" y="745" font-family="${SANS}" font-size="120" font-weight="800" text-anchor="middle" fill="${grey}">Hs</text>
      <text x="340" y="850" font-family="${SANS}" font-size="38" font-weight="700" text-anchor="middle" fill="${grey}">Homo sapiens</text>
      <text x="340" y="895" font-family="${MONO}" font-size="22" letter-spacing="2" text-anchor="middle" fill="${grey}" fill-opacity=".7">FACTORY DEFAULT</text>
      <text x="600" y="730" font-family="${SANS}" font-size="90" font-weight="800" text-anchor="middle" fill="${DB.green}">&#8594;</text>
      <rect x="670" y="500" width="380" height="420" rx="18" fill="none" stroke="${DB.green}" stroke-width="9"/>
      <text x="700" y="575" font-family="${MONO}" font-size="40" fill="${DB.green}">01</text>
      <text x="860" y="745" font-family="${SANS}" font-size="110" font-weight="800" text-anchor="middle" fill="${DB.green}">H+</text>
      <text x="860" y="850" font-family="${SANS}" font-size="38" font-weight="700" text-anchor="middle" fill="${DB.green}">Human plus</text>
      <text x="860" y="895" font-family="${MONO}" font-size="22" letter-spacing="2" text-anchor="middle" fill="${DB.green}" fill-opacity=".8">ENHANCED</text>
    </svg>`;
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1400">
    <text x="600" y="690" font-family="${SANS}" font-size="150" font-weight="800" text-anchor="middle" letter-spacing="8" fill="#fff">HUMAN<tspan fill="${DB.green}">+</tspan></text>
    <text x="600" y="770" font-family="${MONO}" font-size="34" text-anchor="middle" letter-spacing="10" fill="#fff" fill-opacity=".5">MODIFIED &#183; ENHANCED &#183; OPTIMIZED</text>
  </svg>`;
}

/* The BACK print = stack name + tiles + tagline + slogan (single tile if 1
   compound selected). A loaded preset renders as a stack so its extras show. */
function currentBackSVG(){
...
/* A single-compound selection is a one-print design (the tile itself = front);
   a stack/blend uses the split layout: HUMAN+ logo front, stack art back. */
function isSplit(){
  if(selected.length===1 && !presetMeta){const e=EL[selected[0]]; return !!e.blend;}
  return selected.length>1 || !!presetMeta || (selected.length===1 && EL[selected[0]].blend);
}
/* design for the chosen side. side="front" -> logo (split designs only). */
function currentDesignSVG(side){
  side = side || designSide;
  if(side==="front" && isSplit()) return frontDesignSVG();
  return currentBackSVG();
}
...
function buildGarmentPage(){
  ...
  // Front/Back toggle: only meaningful for split (stack/blend) designs.
  const split=isSplit();
  document.getElementById('sideRow').style.display=split?"":"none";
  if(!split) designSide="back";
  document.querySelectorAll('#sideSeg button').forEach(b=>b.classList.toggle('on',b.dataset.side===designSide));
  document.getElementById('sideHint').textContent=split
    ?(designSide==="front"?"Front: HUMAN+ logo (pick a placement)":"Back: your stack design"):"";
  // front-style picker only when viewing the front of a split design
  document.getElementById('frontStyleRow').style.display=(split&&designSide==="front")?"":"none";
  document.querySelectorAll('#frontSeg button').forEach(b=>b.classList.toggle('on',b.dataset.fs===frontStyle));
  // sticker download only for a single plain element (stickers are per-element)
  document.getElementById('downloadStickerBtn').style.display=soloElement()?"":"none";
  ...
  currentArt=recolorArt(currentDesignSVG(designSide),garmentColor,accentColor);
...
document.querySelectorAll('#sideSeg button').forEach(b=>b.onclick=()=>{designSide=b.dataset.side;buildGarmentPage();});
document.querySelectorAll('#frontSeg button').forEach(b=>b.onclick=()=>{frontStyle=b.dataset.fs;buildGarmentPage();});
...
function fileBase(){
  ...
  let sidePart="";   // front/back only for split designs; front carries its style
  if(isSplit()) sidePart = designSide==="front" ? ("_front_"+frontStyle) : "_back";
  return `${nm}${sidePart}_${tone}`;
}
...
// Sticker download: 2400x2400 transparent PNG of the single selected element.
// Honors the garment-color choice (light garment/material -> dark ink) via recolorArt.
function downloadSticker(){
  const e=soloElement(); if(!e){alert("Pick a single element to download its sticker.");return;}
  const art=recolorArt(stickerSVG(e),garmentColor,accentColor).replace(/^<svg /i,'<svg width="2400" height="2400" ');
  const img=new Image();
  const url=URL.createObjectURL(new Blob([art],{type:"image/svg+xml"}));
  img.onload=()=>{
    ...
    const tone=luminance(garmentColor)>0.6?"light":"dark";
    c.toBlob(b=>{const a=document.createElement('a');a.href=URL.createObjectURL(b);
      a.download=`${e.full.replace(/\s+/g,"_")}_sticker_${tone}_2400.png`;a.click();URL.revokeObjectURL(a.href);},"image/png");
  };
  img.onerror=()=>{URL.revokeObjectURL(url);alert("Sticker export failed in this browser.");};
  img.src=url;
}
const stkBtn=document.getElementById('downloadStickerBtn');
if(stkBtn) stkBtn.onclick=downloadSticker;
